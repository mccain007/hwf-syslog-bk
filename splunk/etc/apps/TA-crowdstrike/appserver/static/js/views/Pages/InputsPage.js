...
define([
     ...
     return Backbone.View.extend({
         ...
         initialize: function () {
             ...
             this.accounts_deferred = this.fetchListCollection(this.accounts, this.stateModel);
         ...
         render: function () {
             this.accounts_deferred.done(function () {
                 var title_template, no_account_html, inputs_template_data, temp_collection;
                 if (this.accounts.models.length === 0) {
                     inputs_template_data = {
                         title: ComponentMap.input.caption.title,
                         description: ComponentMap.input.caption.description,
                         enableButton: false,
                         enableHr: ComponentMap.input.caption.enableHr
                     };

                     title_template = _.template(InputTitleTemplate);
                     this.$el.append(title_template(inputs_template_data));
                     no_account_html = '<div style="padding:20px"><p>No inputs. <a href="./configuration">Create an account</a> before creating an input.</p></div>';
                     this.$el.append($(no_account_html));
                 }
                 else {
                     this.$el.append('<div class="load_message">Loading...</div>');
         ...
         sortCollection: function (stateModel) {
             var sort_dir = stateModel.get('sortDirection'),
                 ...
                 handler = {
                     ...
                     'start_offset': sort_numerical,
                     'account': sort_alphabetical,
