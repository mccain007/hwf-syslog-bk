...
REQUESTS_CA_CERT_FILE_NAME = 'cacert.pem'
...
def _get():
    ...
    if TEMP_CERT_FILE_PATH is None or not os.path.exists(TEMP_CERT_FILE_PATH):
        ...
        if ssl_ca_certs:  # only update temp cert file when there's additional PEM certs found
            cert_files = [ssl_ca_certs, _read_requests_default_certs()]
...
def _read_requests_default_certs():
    import requests  # import error should not happen here, and will be well handled by outer called
    requests_dir = os.path.dirname(os.path.abspath(requests.__file__))
    ca_certs_path = os.path.join(requests_dir, REQUESTS_CA_CERT_FILE_NAME)
